...
void blokiran_racun()//dodati oporavak baze tako da stvori datoteku za knjiznicara i da unese zadanu lozniku 1234 ili ne�to
{
    system("cls");
            ...
            if (izborP == 2)
            {
                ...
                while (pokusajP < 3) {
                    ...
                    if (PASSWORDP == PasswordP)
                    {
                        ...
                        korisnik >> kime;
                        korisnik.close();
                        cout << kime << "TRENUTNO JE POSUDZENO: " << knjiga << endl;
                        korisnik.close();
                        //knjiga = '\0';
                        sleep_for(seconds(2));
                        datoteka.close();
